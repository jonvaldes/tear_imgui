...
void ImGui::ShowTestWindow(bool *p_open) {
    ...
    if(show_app_style_editor) {
        ImGui::SetNextWindowPos(ImVec2(4, 6), ImGuiSetCond_FirstUseEver);
        ImGui::SetNextWindowSize(ImVec2(85, 30), ImGuiSetCond_Always);
    ...
    ImGui::SetNextWindowPos(ImVec2(0, 30), ImGuiSetCond_FirstUseEver);
    ImGui::SetNextWindowSize(ImVec2(140, 50), ImGuiSetCond_FirstUseEver);
    ...
    ImGui::PushItemWidth(-40); // Right align, keep 140 pixels for labels

    ImGui::Text("ImGui UI says hello.");
    ...
    if(ImGui::CollapsingHeader("Widgets")) {
        if(ImGui::TreeNode("Trees")) {
            ...
            if(ImGui::TreeNode("Advanced, with Selectable nodes")) {
                ...
                ImGui::PushStyleVar(ImGuiStyleVar_IndentSpacing, 3); // Increase spacing to differentiate leaves from expanded contents.
                for(int i = 0; i < 6; i++) {
        ...
        if(ImGui::TreeNode("Colored Text")) {
            ...
            ImGui::TextColored(ImColor(255, 0), "Pink");
            ImGui::TextColored(ImColor(255, 255), "Yellow");
        ...
        if(ImGui::TreeNode("Word Wrapping")) {
            ...
            ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(pos.x + wrap_width, pos.y), ImVec2(pos.x + wrap_width + 10, pos.y + ImGui::GetTextLineHeight()), ImColor(255,255));
            /*
        ...
        for(int i = 0; i < 7; i++) {
            if(i > 0)
                ImGui::SameLine();
            ImGui::PushID(i);
            ImGui::PushStyleColor(ImGuiCol_Button, ImColor(i * 14, 153));
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImColor(i * 14, 179));
            ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImColor(i * 14, 204));
            ImGui::Button("Click");
        ...
        if(ImGui::TreeNode("Vertical Sliders")) {
            ...
            for(int i = 0; i < 7; i++) {
                if(i > 0)
                    ImGui::SameLine();
                ImGui::PushID(i);
                ImGui::PushStyleColor(ImGuiCol_FrameBg, ImColor(i * 14, 127));
                ImGui::PushStyleColor(ImGuiCol_FrameBgHovered, ImColor(i * 14, 153));
                ImGui::PushStyleColor(ImGuiCol_FrameBgActive, ImColor(i * 14, 204));
                ImGui::PushStyleColor(ImGuiCol_SliderGrab, ImColor(i * 14, 230));
    ...
    if(ImGui::CollapsingHeader("Graphs widgets")) {
        ...
        if(animate) {
            ...
            for(; ImGui::GetTime() > refresh_time + 1.0f / 60.0f; refresh_time += 1.0f / 60.0f) {
                ...
                values[values_offset] = 0.8 * cosf(phase);
                values_offset         = (values_offset + 1) % IM_ARRAYSIZE(values);
                phase += 0.00125f * values_offset;
            }
        }
        ImGui::PlotLines("Lines", values, IM_ARRAYSIZE(values), values_offset, "avg 0.0", -1.0f, 1.0f, ImVec2(0, 20));
        ImGui::PlotHistogram("Histogram", arr, IM_ARRAYSIZE(arr), 0, NULL, 0.0f, 1.0f, ImVec2(0, 30));

        // Use functions to generate output
        // FIXME: This is rather awkward because current plot API only pass in indices. We probably want an API passing floats and user provide sample rate/count.
        struct Funcs {
            static float Sin(void *, int i) { return 0.8 * sinf(i * 0.1f); }
        ...
        ImGui::PlotLines("Lines", func, NULL, display_count, 0, NULL, -1.0f, 1.0f, ImVec2(0, 30));
        ImGui::PlotHistogram("Histogram", func, NULL, display_count, 0, NULL, -1.0f, 1.0f, ImVec2(0, 30));
        ImGui::Separator();
    ...
    if(ImGui::CollapsingHeader("Layout")) {
        ...
        if(ImGui::TreeNode("Widgets Width")) {
            ...
            ImGui::SameLine();
            ShowHelpMarker("Align to right edge minus 40");
            ImGui::PushItemWidth(-40);
        ...
        if(ImGui::TreeNode("Basic Horizontal Layout")) {
            ...
            ImGui::SameLine();
            ImGui::TextColored(ImColor(1, 1), "Sailor");
        ...
        if(ImGui::TreeNode("Scrolling")) {
            ...
            static int  track_line = 50, scroll_to_px = 10;
            ...
            ImGui::SameLine(30);
            scroll_to |= ImGui::DragInt("##pos_y", &scroll_to_px, 1.00f, 0, 50, "y = %.0f px");
            ImGui::PopItemWidth();
            if(scroll_to)
                track = false;
            ...
            for(int i = 0; i < 5; i++) {
                ...
                ImGui::BeginChild(ImGui::GetID((void *)(intptr_t)i), ImVec2(ImGui::GetWindowWidth() * 0.17f, 20.0f), true);
                if(scroll_to)
                    ImGui::SetScrollFromPosY(ImGui::GetCursorStartPos().y + scroll_to_px, i * 0.25f);
                for(int line = 0; line < 100; line++) {
                    if(track && line == track_line) {
                        ImGui::TextColored(ImColor(1, 1), "Line %d", line);
        ...
        if(ImGui::TreeNode("Horizontal Scrolling")) {
            ...
            for(int line = 0; line < lines; line++) {
                ...
                for(int n = 0; n < num_buttons; n++) {
                    ...
                    int hue   = n * 10; 
                    ImGui::PushStyleColor(ImGuiCol_Button, ImColor(hue, 100));
                    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImColor(hue, 140));
                    ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImColor(hue, 200));
        ...
        if(ImGui::TreeNode("Clipping")) {
            static ImVec2 size(40, 30), offset(20, 15);
            ...
            ImGui::GetWindowDrawList()->AddRectFilled(pos, ImVec2(pos.x + size.x, pos.y + size.y), ImColor(90, 90));
            ImGui::GetWindowDrawList()->AddTextEx(ImVec2(pos.x + offset.x, pos.y + offset.y), ImColor(255, 255), "Line 1 hello\nLine 2 clip me!", NULL, 0.0f, &clip_rect);
            ImGui::TreePop();
        }
    }
    ...
    if(ImGui::CollapsingHeader("Popups & Modal windows")) {
        ...
        if(ImGui::TreeNode("Context menus")) {
            ...
            static ImVec4 color = ImVec4(0.8f, 0.5f, 1.0f, 1.0f);
    ...
    if(ImGui::TreeNode("Colors")) {
        ...
        if(ImGui::Button("Copy Colors")) {
            ...
            for(int i = 0; i < ImGuiCol_COUNT; i++) {
                const ImColor &col  = style.Colors[i];
                const char *  name = ImGui::GetStyleColName(i);
                if(!output_only_modified || memcmp(&col, (ref ? &ref->Colors[i] : &default_style.Colors[i]), sizeof(ImColor)) != 0)
                    ImGui::LogText("style.Colors[ImGuiCol_%s]%*s= ImColor(%3d, %3d);" IM_NEWLINE, name, 22 - (int)strlen(name), "", col.fg, col.bg);
            }
            ImGui::LogFinish();
        }
        ImGui::SameLine();
        ...
        for(int i = 0; i < ImGuiCol_COUNT; i++) {
            ...
            if(!filter.PassFilter(name))
                continue;
            ImGui::PushID(i);
            ImGui::ColorEdit4(name, (float *)&style.Colors[i], false);
...
static void ShowExampleAppCustomRendering(bool *p_open) {
    ImGui::SetNextWindowSize(ImVec2(35, 56), ImGuiSetCond_FirstUseEver);
    ...
    ImGui::Separator();
    {
        ...
        for(int i = 0; i < points.Size - 1; i += 2)
            draw_list->AddLine(ImVec2(canvas_pos.x + points[i].x, canvas_pos.y + points[i].y), ImVec2(canvas_pos.x + points[i + 1].x, canvas_pos.y + points[i + 1].y), ImColor(255,0), 2.0f);
        draw_list->PopClipRect();
        if(adding_preview)
            points.pop_back();
    }
...
struct ExampleAppConsole {
    ...
    void Draw(const char *title, bool *p_open) {
        ImGui::SetNextWindowSize(ImVec2(52, 60), ImGuiSetCond_FirstUseEver);
        ...
        for(int i = 0; i < Items.Size; i++) {
            ...
            if(!filter.PassFilter(item))
                continue;
            ImColor col = ImColor(255,255); // A better implementation may store a type per-item. For the sample let's just parse the text.
...
struct ExampleAppLog {
    ...
    void Draw(const char *title, bool *p_open = NULL) {
        ImGui::SetNextWindowSize(ImVec2(50, 40), ImGuiSetCond_FirstUseEver);
...
static void ShowExampleAppLayout(bool *p_open) {
    ImGui::SetNextWindowSize(ImVec2(50, 44), ImGuiSetCond_FirstUseEver);
...
static void ShowExampleAppPropertyEditor(bool *p_open) {
    ImGui::SetNextWindowSize(ImVec2(43, 45), ImGuiSetCond_FirstUseEver);
