...
struct ImDrawChar;                      // A single vertex (20 bytes by default, override layout with IMGUI_OVERRIDE_DRAWVERT_STRUCT_LAYOUT)
...
typedef int            ImI32;                // 32-bit unsigned integer (typically used to store packed colors)
...
IMGUI_API const ImColor &GetImColor(ImGuiCol idx); // retrieve given style color with style alpha applied and optional extra alpha multiplier
IMGUI_API ImU32 GetColorU32(const ImVec4 &col);                    // retrieve given color with style alpha applied

inline int GetColorValue(float r, float g, float b){
	if(r == g && g == b){
		// Grayscale ramp
		return 0xe8 + (0xff - 0xe8) * r;
	}
	// We have 0xD7 RGB colors.
	unsigned char ur = (unsigned char)(r * 6);
	unsigned char ug = (unsigned char)(g * 6);
	unsigned char ub = (unsigned char)(b * 6);
	return 0x10 + ur * 36 + ug * 6 + ub;
}
...
IMGUI_API void TextColored(const ImColor &col, const char *fmt, ...) IM_PRINTFARGS(2); // shortcut for PushStyleColor(ImGuiCol_Text, col); Text(fmt, ...); PopStyleColor();
IMGUI_API void TextColoredV(const ImColor &col, const char *fmt, va_list args);
...
struct ImDrawList {
    ...
    ImVector<ImDrawChar> VtxBuffer; // Vertex buffer.

    // [Internal, used while building lists]
    const char *            _OwnerName;       // Pointer to owner window's name (if any) for debugging
    unsigned int            _VtxCurrentIdx;   // [Internal] == VtxBuffer.Size
    ImDrawChar *            _VtxWritePtr;     // [Internal] point within VtxBuffer.Data after each add command (to avoid using the ImVector<> operators too much)
    ...
    // Primitives
    IMGUI_API void AddLine(const ImVec2 &a, const ImVec2 &b, const ImColor &col, int ch = L'─');
    IMGUI_API void AddRectFilled(const ImVec2 &a, const ImVec2 &b, const ImColor &col, float rounding = 0.0f, int rounding_corners = 0x0F); // a: upper-left, b: lower-right
    ...
    IMGUI_API void AddTextEx(const ImVec2 &pos, const ImColor &col, const char *text_begin, const char *text_end = NULL, float wrap_width = 0.0f, const ImVec4 *cpu_fine_clip_rect = NULL);
    IMGUI_API void AddChar(const ImVec2 &pos, const ImColor &col, int ch);
    //IMGUI_API void AddPolyline(const ImVec2 *points, const int num_points, const ImColor& col, bool closed, float thickness, bool anti_aliased);
    IMGUI_API void AddConvexPolyFilled(const ImVec2 *points, const int num_points, const ImColor &col, bool anti_aliased);
    ...
    inline void PathFill(const ImColor &col) {
    ...
    inline void PrimWriteChar(const ImVec2 &pos, ImI32 c, const ImColor &col) {
    ...
    inline void PrimChar(const ImVec2 &pos, ImI32 c, const ImColor &col) {
        PrimWriteIdx((ImDrawIdx)_VtxCurrentIdx);
        PrimWriteChar(pos, c, col);
    }
