...
const ImColor &ImGui::GetImColor(ImGuiCol idx) {
...
void ImGui::Render() {
    ...
    if(g.Style.Alpha > 0.0f) {
        ...
        // Setup draw data
        ImDrawList **CmdLists      = (g.RenderDrawLists[0].Size > 0) ? &g.RenderDrawLists[0][0] : NULL;
        int          CmdListsCount = g.RenderDrawLists[0].Size;
        int          TotalVtxCount = g.IO.MetricsRenderVertices;

        if(CmdListsCount > 0) {
            int      width  = tb_width();
            int      height = tb_height();
            tb_cell *cells  = tb_cell_buffer();
            memset(cells, 0, sizeof(tb_cell) * width * height);

            // TODO (jon): Render everything with termbox here
            for(int i = 0; i < CmdListsCount; i++) {
                ImDrawList *list       = CmdLists[i];
                int         drawnVerts = 0;
                for(int cmd = 0; cmd < list->CmdBuffer.Size; cmd++) {
                    const ImDrawCmd *pcmd = &list->CmdBuffer[cmd];
                    ImVec4           rect = pcmd->ClipRect;
                    for(int v = 0; v < pcmd->ElemCount; v++) {
                        ImDrawChar c = list->VtxBuffer[v + drawnVerts];
                        ...
                        ImColor color = c.col;
                        if(color.bg == 0) {
                            color.bg = cells[idx].bg;
                        }
                        cells[idx].fg = color.fg;
                        cells[idx].bg = color.bg;
                        cells[idx].ch = c.c;
                    }
                    drawnVerts += pcmd->ElemCount;
                }
            }
        }
    }
...
void ImGui::RenderText(ImVec2 pos, const char *text, const char *text_end, bool hide_text_after_hash) {
    ...
    if(text_len > 0) {
        window->DrawList->AddTextEx(pos, GetImColor(ImGuiCol_Text), text, text_display_end);
        if(g.LogEnabled)
...
void ImGui::RenderTextWrapped(ImVec2 pos, const char *text, const char *text_end, float wrap_width) {
    ...
    if(text_len > 0) {
        window->DrawList->AddTextEx(pos, GetImColor(ImGuiCol_Text), text, text_end, wrap_width);
        if(g.LogEnabled)
...
void ImGui::RenderTextClipped(const ImVec2 &pos_min, const ImVec2 &pos_max, const char *text, const char *text_end, const ImVec2 *text_size_if_known, const ImVec2 &align, const ImRect *clip_rect) {
    ...
    if(need_clipping) {
        ...
        window->DrawList->AddTextEx(pos, GetImColor(ImGuiCol_Text), text, text_display_end, 0.0f, &fine_clip_rect);
    } else {
        window->DrawList->AddTextEx(pos, GetImColor(ImGuiCol_Text), text, text_display_end, 0.0f, NULL);
    }
    if(g.LogEnabled)
        LogRenderedText(pos, text, text_display_end);
}
...
void ImGui::RenderFrame(ImVec2 p_min, ImVec2 p_max, const ImColor &fill_col, bool border, float rounding) {
	...
	if(border){
		...
		window->DrawList->AddQuadFilled(p_min, p_max, p_min, p_max, fill_col);
	}
}
...
void ImGui::RenderCollapseTriangle(ImVec2 p_min, bool is_open, float scale, bool shadow) {
    ImGuiContext &g      = *GImGui;
    ImGuiWindow * window = GetCurrentWindow();
    window->DrawList->AddChar(p_min, GetImColor(ImGuiCol_Text), is_open ? L'▼' : L'▶');
}

void ImGui::RenderBullet(ImVec2 pos) {
    ImGuiWindow *               window = GetCurrentWindow();
    window->DrawList->AddChar(pos, GetImColor(ImGuiCol_Text), '*');
    /*
    window->DrawList->AddCircleFilled(pos, GImGui->FontSize*0.20f, GetImColor(ImGuiCol_Text), 8);
	*/
}

void ImGui::RenderCheckMark(ImVec2 pos, const ImColor &col) {

    ImGuiWindow *window = GetCurrentWindow();
    window->DrawList->AddChar(pos, col, L'X');
}
...
ImVec2 ImGui::CalcTextSize(const char *text, const char *text_end, bool hide_text_after_double_hash, float wrap_width) {
    ...
    const float font_size = 1.f;
    if(text == text_display_end)
        return ImVec2(0.0f, font_size);
    ImVec2 text_size = TrCalcTextSizeA(font_size, FLT_MAX, wrap_width, text, text_display_end, NULL);
...
bool ImGui::Begin(const char *name, bool *p_open, const ImVec2 &size_on_first_use, float bg_alpha, ImGuiWindowFlags flags) {
    ...
    if(first_begin_of_the_frame) {
        ...
        if(window->Size.x > 0.0f && !(flags & ImGuiWindowFlags_Tooltip) && !(flags & ImGuiWindowFlags_AlwaysAutoResize))
            ...
            window->ItemWidthDefault = 16.f; //(float)(int)(g.FontSize * 16.0f);
        ...
        if((flags & ImGuiWindowFlags_Modal) != 0 && window == GetFrontMostModalRootWindow())
            window->DrawList->AddRectFilled(fullscreen_rect.Min, fullscreen_rect.Max, GetImColor(ImGuiCol_ModalWindowDarkening));
        ...
        if(window->Collapsed) {
            // Draw title bar only
            RenderFrame(title_bar_rect.GetTL(), title_bar_rect.GetBR(), GetImColor(ImGuiCol_TitleBgCollapsed), true, window_rounding);
        } else {
            ImColor resize_col = ImColor();

            const float resize_corner_size = ImMax(1.35f, window_rounding + 1.0f + 0.2f);
            if(!(flags & ImGuiWindowFlags_AlwaysAutoResize) && window->AutoFitFramesX <= 0 && window->AutoFitFramesY <= 0 && !(flags & ImGuiWindowFlags_NoResize)) {
                ...
                resize_col = GetImColor(held ? ImGuiCol_ResizeGripActive : hovered ? ImGuiCol_ResizeGripHovered : ImGuiCol_ResizeGrip);
            ...
            ImColor bg_color = style.Colors[bg_color_idx];

            window->DrawList->AddRectFilled(window->Pos, window->Pos + window->Size, bg_color, window_rounding, (flags & ImGuiWindowFlags_NoTitleBar) ? 15 : 4 | 8);

            // Menu bar
            if(flags & ImGuiWindowFlags_MenuBar) {
                ImRect menu_bar_rect = window->MenuBarRect();
				window->DrawList->AddQuadFilled(menu_bar_rect.GetTL(), menu_bar_rect.GetBR(), menu_bar_rect.GetTL(), menu_bar_rect.GetBR(), GetImColor(ImGuiCol_MenuBarBg));
            }
        ...
        window->ContentsRegionRect.Min.y = -window->Scroll.y + window->WindowPadding.y + window->MenuBarHeight();
        ...
        window->DC.CursorStartPos    = window->Pos + ImVec2(window->DC.IndentX + window->DC.ColumnsOffsetX, window->MenuBarHeight() + window->WindowPadding.y - window->Scroll.y);
        ...
        if(!(flags & ImGuiWindowFlags_NoTitleBar)) {
            ...
            if(!(flags & ImGuiWindowFlags_NoCollapse))
                RenderCollapseTriangle(window->Pos + ImVec2(1, 0) + style.FramePadding, !window->Collapsed, 1.0f, true);
            ...
            float pad_left  = (flags & ImGuiWindowFlags_NoCollapse) == 0 ? (style.FramePadding.x + /*g.FontSize*/ 1.f + style.ItemInnerSpacing.x) : 0.0f;
            float pad_right = (p_open != NULL) ? (style.FramePadding.x + /*g.FontSize*/ 1.f + style.ItemInnerSpacing.x) : 0.0f;
...
static void Scrollbar(ImGuiWindow *window, bool horizontal) {
    ...
    //window->DrawList->AddRectFilled(bb.Min, bb.Max, ImGui::GetImColor(ImGuiCol_ScrollbarBg), window_rounding, window_rounding_corners);
	window->DrawList->AddLine(bb.Min, ImVec2(bb.Min.x, bb.Max.y), ImGui::GetImColor(ImGuiCol_ScrollbarBg), L'┃');
    ...
    const int grab_h_pixels = 0.5f + ImMin(ImMax(scrollbar_size_v * ImSaturate(win_size_avail_v / ImMax(win_size_contents_v, win_size_avail_v)), style.GrabMinSize), scrollbar_size_v);
    const int grab_h_norm   = grab_h_pixels / scrollbar_size_v;
    ...
    // Render
    const ImColor &grab_col = ImGui::GetImColor(held ? ImGuiCol_ScrollbarGrabActive : hovered ? ImGuiCol_ScrollbarGrabHovered : ImGuiCol_ScrollbarGrab);
    if(horizontal) {
		...
		window->DrawList->AddLine(ImVec2(bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm)-3), ImVec2(bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, grab_v_norm) + grab_h_pixels+3), grab_col, L'█');
    }
}
...
void ImGui::TextColoredV(const ImColor &col, const char *fmt, va_list args) {
    PushStyleColor(ImGuiCol_Text, col);
    TextV(fmt, args);
    PopStyleColor();
}

void ImGui::TextColored(const ImColor &col, const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
...
void ImGui::TextUnformatted(const char *text, const char *text_end) {
    ...
    if(text_end - text > 2000 && !wrap_enabled) {
        ...
        if(text_pos.y <= clip_rect.Max.y) {
            ...
            // Lines to render
            if(line < text_end) {
                ImRect line_rect(pos, pos + ImVec2(FLT_MAX, 1));
                while(line < text_end) {
                    const char *line_end = strchr(line, '\n');
...
void ImGui::AlignFirstTextHeightToWidgets() {
    ...
    ImGuiContext &g = *GImGui;
    ItemSize(ImVec2(0, 1.f + g.Style.FramePadding.y * 2), g.Style.FramePadding.y);
    SameLine(0, 0);
}
...
bool ImGui::ButtonEx(const char *label, const ImVec2 &size_arg, ImGuiButtonFlags flags) {
    ...
    ImVec2 size = CalcItemSize(size_arg, label_size.x + 4, label_size.y + 2);
    ...
    // Render
    const ImColor &col = GetImColor((hovered && held) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderFrame(bb.Min, bb.Max - ImVec2(0, 1), col, true, 0);
    RenderTextClipped(bb.Min + ImVec2(0, 1), bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb);
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2 &pos, float radius) {
    ...
    // Render
    const ImColor &col    = GetImColor((held && hovered) ? ImGuiCol_CloseButtonActive : hovered ? ImGuiCol_CloseButtonHovered : ImGuiCol_CloseButton);
    const ImVec2   center = bb.GetCenter();
    window->DrawList->AddCircleFilled(center, ImMax(2.0f, radius), col, 12);

    const float cross_extent = (radius * 0.7071f) - 1.0f;
    if(hovered) {
        window->DrawList->AddLine(center + ImVec2(+cross_extent, +cross_extent), center + ImVec2(-cross_extent, -cross_extent), GetImColor(ImGuiCol_Text));
        window->DrawList->AddLine(center + ImVec2(+cross_extent, -cross_extent), center + ImVec2(-cross_extent, +cross_extent), GetImColor(ImGuiCol_Text));
    }
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char *label, const char *label_end) {
    ...
    const float frame_height       = ImMax(1.f, label_size.y);
    ...
    const float text_offset_x = (1.f + (display_frame ? padding.x * 3 : padding.x * 2));           // Collapser arrow width + Spacing
    const float text_width    = 1.f + (label_size.x > 0.0f ? label_size.x + padding.x * 2 : 0.0f); // Include collapser
    ...
    // Render
    const ImColor &col      = GetImColor((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header);
...
float ImGui::GetTreeNodeToLabelSpacing() {
    ImGuiContext &g = *GImGui;
    return /*g.FontSize*/ 1.f + (g.Style.FramePadding.x * 2.0f);
}
...
void ImGui::Bullet() {
    ...
    const ImRect      bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(1.f, line_height));
    ItemSize(bb);
    if(!ItemAdd(bb, NULL)) {
    ...
    RenderBullet(bb.Min + ImVec2(style.FramePadding.x + /*g.FontSize*/ 0.5f, line_height));
    SameLine(0, 1);
}
...
void ImGui::BulletTextV(const char *fmt, va_list args) {
    ...
    const float  line_height        = 1.f;
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(1.f + (label_size.x > 0.0f ? (label_size.x + style.FramePadding.x * 2) : 0.0f), ImMax(line_height, label_size.y))); // Empty text doesn't add padding
    ItemSize(bb);
    if(!ItemAdd(bb, NULL))
        return;
    ...
    RenderText(bb.Min + ImVec2(/*g.FontSize*/ 1.f + style.FramePadding.x * 2, text_base_offset_y), text_begin, text_end, false);
}
...
bool ImGui::SliderBehavior(const ImRect &frame_bb, ImGuiID id, float *v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags) {
    ...
    // Draw frame
    //RenderFrame(frame_bb.Min, frame_bb.Max, GetImColor(ImGuiCol_FrameBg), true, style.FrameRounding);
    float lineHeight = frame_bb.Min.y;
    window->DrawList->AddLine(ImVec2(frame_bb.Min.x + 1, lineHeight), ImVec2(frame_bb.Max.x, lineHeight), GetImColor(ImGuiCol_Border));
    window->DrawList->AddChar(ImVec2(frame_bb.Min.x, lineHeight), GetImColor(ImGuiCol_Border), L'⁅');
    window->DrawList->AddChar(ImVec2(frame_bb.Max.x, lineHeight), GetImColor(ImGuiCol_Border), L'⁆');
    ...
    const ImColor &color = GetImColor(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab);
    window->DrawList->AddChar(grab_bb.Min - ImVec2(0, 2), color, L'╫');

    //window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetImColor(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
...
bool ImGui::SliderFloat(const char *label, float *v, float v_min, float v_max, const char *display_format, float power) {
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true) + ImVec2(0, 1);
    ...
    char *p             = value_buf;
    *p++                = '[';
    char *value_buf_end = p + ImFormatString(p, IM_ARRAYSIZE(value_buf) - 1, display_format, *v);
    *value_buf_end++    = ']';

    RenderTextClipped(frame_bb.Min + ImVec2(0, 1), frame_bb.Max + ImVec2(0, 1), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
...
bool ImGui::DragBehavior(const ImRect &frame_bb, ImGuiID id, float *v, float v_speed, float v_min, float v_max, int decimal_precision, float power) {
    ...
    // Draw frame
    const ImColor &frame_col = GetImColor(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    //RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(frame_col.bg, frame_col.bg), true, style.FrameRounding);
...
bool ImGui::DragFloat(const char *label, float *v, float v_speed, float v_min, float v_max, const char *display_format, float power) {
    ...
    const ImRect inner_bb(frame_bb.Min + style.FramePadding + ImVec2(0, 1), frame_bb.Max - style.FramePadding - ImVec2(0, 1));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    ItemSize(total_bb, style.FramePadding.y);
    const ImColor &frame_col = GetImColor(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    window->DrawList->AddQuadFilled(inner_bb.Min, inner_bb.Max, inner_bb.Min, inner_bb.Max, frame_col);
...
void ImGui::PlotEx(ImGuiPlotType plot_type, const char *label, float (*values_getter)(void *data, int idx), void *data, int values_count, int values_offset, const char *overlay_text, float scale_min, float scale_max, ImVec2 graph_size) {
    ...
    const ImColor &col_base    = GetImColor((plot_type == ImGuiPlotType_Lines) ? ImGuiCol_PlotLines : ImGuiCol_PlotHistogram);
    const ImColor &col_hovered = GetImColor((plot_type == ImGuiPlotType_Lines) ? ImGuiCol_PlotLinesHovered : ImGuiCol_PlotHistogramHovered);
...
void ImGui::ProgressBar(float fraction, const ImVec2 &size_arg, const char *overlay) {
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImRect bb(pos, pos + CalcItemSize(size_arg, CalcItemWidth(), /*g.FontSize*/ 1.f + style.FramePadding.y * 2.0f));
    ImRect size_bb(pos, pos + CalcItemSize(size_arg, CalcItemWidth(), /*g.FontSize*/ 2.f + style.FramePadding.y * 2.0f));
    ItemSize(size_bb, style.FramePadding.y);
    if(!ItemAdd(bb, NULL))
        return;

    // Render
    fraction = ImSaturate(fraction);
    RenderFrame(bb.Min, bb.Max, GetImColor(ImGuiCol_FrameBg), true, style.FrameRounding);
    bb.Reduce(ImVec2(window->BorderSize, window->BorderSize));
    const ImVec2 fill_br = ImVec2(ImLerp(bb.Min.x, bb.Max.x, fraction), bb.Max.y);
    RenderFrame(bb.Min, fill_br, GetImColor(ImGuiCol_PlotHistogram), true, style.FrameRounding);
...
bool ImGui::Checkbox(const char *label, bool *v) {
    ...
    const char *check = *v ? "[x]" : "[ ]";
    RenderText(check_bb.Min, check);
...
bool ImGui::RadioButton(const char *label, bool active) {
    ...
    window->DrawList->AddCircleFilled(center, radius, GetImColor((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), 16);
    if(active) {
        const float check_sz = ImMin(check_bb.GetWidth(), check_bb.GetHeight());
        const float pad      = ImMax(1.0f, (float)(int)(check_sz / 6.0f));
        window->DrawList->AddCircleFilled(center, radius - pad, GetImColor(ImGuiCol_CheckMark), 16);
    }

    if(window->Flags & ImGuiWindowFlags_ShowBorders) {
        window->DrawList->AddCircle(center + ImVec2(1, 1), radius, GetImColor(ImGuiCol_BorderShadow), 16);
        window->DrawList->AddCircle(center, radius, GetImColor(ImGuiCol_Border), 16);
    }
...
bool ImGui::InputTextEx(const char *label, char *buf, int buf_size, const ImVec2 &size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void *user_data) {
    ...
    const ImGuiID id         = window->GetID(label);
    const ImVec2  label_size = CalcTextSize(label, NULL, true);
    ImVec2        size       = CalcItemSize(size_arg, CalcItemWidth(), (is_multiline ? GetTextLineHeight() * 8.0f : label_size.y) + style.FramePadding.y * 2.0f + 1); // Arbitrary default of 8 lines high for multi-line
    const ImRect  frame_bb(window->DC.CursorPos, window->DC.CursorPos + size);
    const ImRect  total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? (style.ItemInnerSpacing.x + label_size.x) : 0.0f, 0.0f));
    ...
    if(is_password) {
        /* TODO (jon): replace characters in password text with '*'
    ...
    if(g.ActiveId == id) {
        ...
        if(IsKeyPressedMap(ImGuiKey_LeftArrow)) {
            ...
            if(io.KeyCtrl)
                SetWindowScrollY(draw_window, ImMax(draw_window->Scroll.y - /*g.FontSize*/ 1.f, 0.0f));
            else
            ...
            if(io.KeyCtrl)
                SetWindowScrollY(draw_window, ImMin(draw_window->Scroll.y + /*g.FontSize*/ 1.f, GetScrollMaxY()));
            else
    ...
    if(!is_multiline)
        draw_window->DrawList->AddLine(frame_bb.Min + ImVec2(0, 1), frame_bb.Max, GetImColor(ImGuiCol_Text), L'▔');
    // RenderFrame(frame_bb.Min, frame_bb.Max, GetImColor(ImGuiCol_FrameBg), true, style.FrameRounding);
    ...
    if(g.ActiveId == id || is_currently_scrolling) {
...

        {
            ...
            cursor_offset.y = searches_result_line_number[0];
            ...
            if(is_multiline)
                text_size = ImVec2(size.x, line_count);
        }
        ...
        if(edit_state.CursorFollow) {
            ...
            if(is_multiline) {
                ...
                if(cursor_offset.y - /*g.FontSize*/ 1.f < scroll_y)
                    scroll_y = ImMax(0.0f, cursor_offset.y - /*g.FontSize*/ 1.f);
        ...
        if(edit_state.StbState.select_start != edit_state.StbState.select_end) {
            ...
            const ImColor &bg_color = GetImColor(ImGuiCol_TextSelectedBg);

            ImVec2 rect_pos = render_pos + select_start_offset - render_scroll;
            for(const ImWchar *p = text_selected_begin; p < text_selected_end;) {
                if(rect_pos.y > clip_rect.w + /*g.FontSize*/ 1.f)
                    break;
                if(rect_pos.y < clip_rect.y) {
                    ...
                    if(rect_size.x <= 0.0f)
                        rect_size.x = 1.f; // So we can see selected empty lines
                    ImRect rect(rect_pos + ImVec2(0.0f, bg_offy_up - /*g.FontSize*/ 1.f), rect_pos + ImVec2(rect_size.x, bg_offy_dn));
                    rect.Clip(clip_rect);
                    if(rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min, rect.Max, bg_color);
                }
                rect_pos.x = render_pos.x - render_scroll.x;
                rect_pos.y += /*g.FontSize*/ 1.f;
            }
        }

        draw_window->DrawList->AddTextEx(render_pos - render_scroll, GetImColor(ImGuiCol_Text), buf_display, buf_display + edit_state.CurLenA, 0.0f, is_multiline ? NULL : &clip_rect);

        // Draw blinking cursor
        bool   cursor_is_visible = (g.InputTextState.CursorAnim <= 0.0f) || fmodf(g.InputTextState.CursorAnim, 1.20f) <= 0.80f;
        ImVec2 cursor_screen_pos = render_pos + cursor_offset - render_scroll;
        ImRect cursor_screen_rect(cursor_screen_pos.x, cursor_screen_pos.y - /*g.FontSize*/ 1.f + 0.5f, cursor_screen_pos.x + 1.0f, cursor_screen_pos.y - 1.5f);
        if(cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
            draw_window->DrawList->AddLine(cursor_screen_rect.Min, cursor_screen_rect.GetBL(), GetImColor(ImGuiCol_Text));

        // Notify OS of text input position for advanced IME (-1 x offset so that Windows IME can cover our cursor. Bit of an extra nicety.)
        if(is_editable)
            g.OsImePosRequest = ImVec2(cursor_screen_pos.x - 1, cursor_screen_pos.y - /*g.FontSize*/ 1.f);
    } else {
        // Render text only
        const char *buf_end = NULL;
        if(is_multiline)
            text_size = ImVec2(size.x, InputTextCalcTextLenAndLineCount(buf_display, &buf_end) * /*g.FontSize*/ 1.f); // We don't need width
        draw_window->DrawList->AddTextEx(render_pos, GetImColor(ImGuiCol_Text), buf_display, buf_end, 0.0f, is_multiline ? NULL : &clip_rect);
    }

    if(is_multiline) {
        Dummy(text_size + ImVec2(0.0f, /*g.FontSize*/ 1.f)); // Always add room to scroll an extra line
        EndChildFrame();
        EndGroup();
    }
...
bool ImGui::InputScalarEx(const char *label, ImGuiDataType data_type, void *data_ptr, void *step_ptr, void *step_fast_ptr, const char *scalar_format, ImGuiInputTextFlags extra_flags) {
    ...
    BeginGroup();
    PushID(label);
    const ImVec2 button_sz = ImVec2(/*g.FontSize*/ 1.f, /*g.FontSize*/ 1.f) + style.FramePadding * 2.0f;
    if(step_ptr)
...
bool ImGui::Combo(const char *label, int *current_item, bool (*items_getter)(void *, int, const char **), void *data, int items_count, int height_in_items) {
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 1.0f));
    ItemSize(total_bb, style.FramePadding.y);
    if(!ItemAdd(total_bb, &id))
        return false;

    const float arrow_size       = (/*g.FontSize*/ 1.f + style.FramePadding.x * 2.0f);
    const bool  hovered          = IsHovered(frame_bb, id);
    ...
    RenderFrame(frame_bb.Min, frame_bb.Max, GetImColor(ImGuiCol_FrameBg), true, style.FrameRounding);
    RenderFrame(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), frame_bb.Max, GetImColor(popup_open || hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button), true, style.FrameRounding); // FIXME-ROUNDING
...
bool ImGui::MenuItem(const char *label, const char *shortcut, bool selected, bool enabled) {
    ...
    float         w             = window->MenuColumns.DeclColumns(label_size.x, shortcut_size.x, (float)(int)(/*g.FontSize*/ 1.f * 1.20f)); // Feedback for next frame
    ...
    if(selected)
        RenderCheckMark(pos + ImVec2(window->MenuColumns.Pos[2] + extra_w + /*g.FontSize*/ 1.f * 0.20f, 0.0f), GetImColor(enabled ? ImGuiCol_Text : ImGuiCol_TextDisabled));
...
bool ImGui::BeginMainMenuBar() {
    ...
    SetNextWindowSize(ImVec2(g.IO.DisplaySize.x, 1));
...
bool ImGui::BeginMenu(const char *label, bool enabled) {
    ...
    if(window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
        ...
        float w       = window->MenuColumns.DeclColumns(label_size.x, 0.0f, (float)(int)(/*g.FontSize*/ 1.f * 1.20f)); // Feedback to next frame
        float extra_w = ImMax(0.0f, GetContentRegionAvail().x - w);
        pressed       = Selectable(label, menu_is_open, ImGuiSelectableFlags_Menu | ImGuiSelectableFlags_DontClosePopups | ImGuiSelectableFlags_DrawFillAvailWidth | (!enabled ? ImGuiSelectableFlags_Disabled : 0), ImVec2(w, 0.0f));
        if(!enabled)
            PushStyleColor(ImGuiCol_Text, g.Style.Colors[ImGuiCol_TextDisabled]);
        RenderCollapseTriangle(pos + ImVec2(window->MenuColumns.Pos[2] + extra_w + /*g.FontSize*/ 1.f * 0.20f, 0.0f), false);
        if(!enabled)
            PopStyleColor();
    }
...
bool ImGui::ColorButton(const ImVec4 &col, bool small_height, bool outline_border) {
    ...
    const float       square_size = /*g.FontSize*/ 3.f;
    const ImRect      bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(square_size, 3));
    ...
    bool    hovered, held;
    bool    pressed = ButtonBehavior(bb, id, &hovered, &held);
    ImColor imcol   = ImColor(0, GetColorValue(col.x, col.y, col.z));
    window->DrawList->AddRectFilled(bb.Min, bb.Max, imcol, outline_border, style.FrameRounding);
...
bool ImGui::ColorEdit4(const char *label, float col[4], bool alpha) {
    ...
    const float       square_sz = (1 + style.FramePadding.y * 2.0f);
...
void ImGui::Separator() {
    ...
    const ImRect bb(ImVec2(x1, window->DC.CursorPos.y + 1), ImVec2(x2, window->DC.CursorPos.y + 1));
    ItemSize(ImVec2(0.0f, 3.0f)); // NB: we don't provide our width so that it doesn't get feed back into AutoFit   // FIXME: Height should be 1.0f not 0.0f ?
    if(!ItemAdd(bb, NULL)) {
        if(window->DC.ColumnsCount > 1)
            PushColumnClipRect();
        return;
    ...
    window->DrawList->AddLine(bb.Min + ImVec2(1, 0), bb.Max, GetImColor(ImGuiCol_Border));
    window->DrawList->AddChar(bb.Min, GetImColor(ImGuiCol_Border), L'├');
    window->DrawList->AddChar(bb.Max, GetImColor(ImGuiCol_Border), L'┤');
...
void ImGui::Columns(int columns_count, const char *id, bool border) {
    ...
    if(window->DC.ColumnsCount != columns_count && window->DC.ColumnsCount != 1 && window->DC.ColumnsShowBorders && !window->SkipItems) {
        ...
        for(int i = 1; i < window->DC.ColumnsCount; i++) {
            ...
            const ImColor &col = GetImColor(held ? ImGuiCol_ColumnActive : hovered ? ImGuiCol_ColumnHovered : ImGuiCol_Column);
    ...
    if(ImGui::Begin("ImGui Metrics", p_open)) {
        ...
        struct Funcs {
            static void NodeDrawList(ImDrawList *draw_list, const char *label) {
                bool node_open = ImGui::TreeNode(draw_list, "%s: '%s' %d vtx, %d cmds", label, draw_list->_OwnerName ? draw_list->_OwnerName : "", draw_list->VtxBuffer.Size, draw_list->CmdBuffer.Size);
                if(draw_list == ImGui::GetWindowDrawList()) {
                    ImGui::SameLine();
                    ImGui::TextColored(ImColor(255, 100), "CURRENTLY APPENDING"); // Can't display stats for active draw list! (we don't have the data double-buffered)
                    if(node_open)
                        ImGui::TreePop();
                    return;
                }
                ...
                for(const ImDrawCmd *pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount, pcmd++) {
                    ...
                    bool pcmd_node_open = ImGui::TreeNode((void *)(pcmd - draw_list->CmdBuffer.begin()), "Draw %-4d %s vtx, clip_rect = (%.0f,%.0f)..(%.0f,%.0f)", pcmd->ElemCount, "non-indexed", pcmd->ClipRect.x, pcmd->ClipRect.y, pcmd->ClipRect.z, pcmd->ClipRect.w);
                    if(show_clip_rects && ImGui::IsItemHovered()) {
                        ImRect clip_rect = pcmd->ClipRect;
                        ImRect vtxs_rect;
                        for(int i = elem_offset; i < elem_offset + (int)pcmd->ElemCount; i++)
                            vtxs_rect.Add(draw_list->VtxBuffer[i].pos);
                        clip_rect.Floor();
                    ...
                    while (clipper.Step())
                        for (int prim = clipper.DisplayStart, vtx_i = elem_offset + clipper.DisplayStart*3; prim < clipper.DisplayEnd; prim++)
                        {
                            ...
                            {
                                ImDrawChar& c = draw_list->VtxBuffer[vtx_i];
