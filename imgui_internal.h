...
struct IMGUI_API ImGuiWindow {
  ...
  public:
    ...
    float  CalcFontSize() const { return 1.f; }
    float  TitleBarHeight() const { return (Flags & ImGuiWindowFlags_NoTitleBar) ? 0.0f : 1.f; }
    ImRect TitleBarRect() const { return ImRect(Pos, ImVec2(Pos.x + SizeFull.x, Pos.y + TitleBarHeight())); }
    float  MenuBarHeight() const { return (Flags & ImGuiWindowFlags_MenuBar) ? 1.f : 0.0f; }
    ImRect MenuBarRect() const {
        float y1 = Pos.y + TitleBarHeight();
        return ImRect(Pos.x + 1, y1, Pos.x + SizeFull.x, y1 + MenuBarHeight());
    }
};
...
IMGUI_API void RenderCheckMark(ImVec2 pos, const ImColor& col);
